...
function SH_ACC:ChangeEquipment(ply, id, eq)
	...
	if (!ply:SH_NearAccessoryVendor() and !ply:SH_HasAccessoryChanger()) then
		return end
	...
	local delayed = !ply:SH_NearAccessoryVendor() and ply:SH_HasAccessoryChanger()
	...
	local function PerformSwitch()
		if (eq) then -- Equip
			...
			if (ply:SH_EquipAccessory(id)) then
				-- get rid of any accessory with conflicting slots
				local uneq = {}
				for oid in pairs (ply.SH_AccessoryInfo.equipped) do
					if (oid == id) then
						continue end
	...
	if (delayed) then
		local t = eq and self.DelayedEquipTime or self.DelayedUnequipTime
	
		local id = "SH_ACC." .. ply:UserID() .. "_DELAY_EQUIP"
		timer.Destroy(id)
		timer.Create(id, t, 1, function()
...
hook.Add("InitPostEntity", "SH_ACC.InitPostEntity", function()
	for _, v in pairs (SH_ACC.NPCSpawns) do
		local e = ents.Create("npc_accessory_vendor")
		e:SetPos(v.pos)
		e:SetAngles(v.ang)
		e:Spawn()
		e:DropToFloor()
...
hook.Add("PlayerSay", "SH_ACC.PlayerSay", function(ply, say, bteam)
	local s = SH_ACC:PlayerSay(ply, say, bteam)
	if (s) then
		return s
	end
end)
